...
# 信号
signal save_created(save_name: String, metadata: Dictionary)  # 存档创建
signal save_loaded(save_name: String, metadata: Dictionary)  # 存档加载
signal save_deleted(save_name: String)  # 存档删除
signal auto_save_created(save_name: String)  # 自动存档创建

# 配置属性
@export var save_directory: String:
	get:
		return ProjectSettings.get_setting(SETTING_SAVE_SYSTEM_DEFAULTS + "save_directory", "user://saves")
	set(_value):
		push_error("read-only")

@export var save_group: String:
	get:
		return ProjectSettings.get_setting(SETTING_SAVE_SYSTEM_DEFAULTS + "save_group", "saveable")
	set(_value):
		push_error("read-only")
...
@export var auto_save_prefix: String:
	get:
		return ProjectSettings.get_setting(SETTING_SAVE_SYSTEM_AUTO_SAVE + "name_prefix", "auto_")
	set(_value):
		push_error("read-only")
...
func _process(delta: float) -> void:
	if auto_save_enabled and not _current_save_name.is_empty():
...
func create_save(save_name: String = "") -> bool:
	...
	if success:
		_current_save_name = actual_name
		_current_id = save_id
		save_created.emit(actual_name, save_data.metadata)
		return true
	return false
...
func load_save(save_name: String) -> bool:
	...
	if not result.is_empty():
		...
		_current_id = result["metadata"].get("id", _get_auto_save_id(save_name))
		if result.has("nodes"):
			_apply_node_states(result.nodes)
		save_loaded.emit(save_name, result.metadata)
		return true
	return false
...
func create_auto_save(auto_save_name: String = "") -> String:
	# 自动计时器归零
	_auto_save_timer = 0
	if auto_save_name.is_empty():
		auto_save_name = _get_auto_save_name()

	# 创建新存档
	var success = await create_save(auto_save_name)
	if not success:
		push_error("Failed to create auto save: %s" % auto_save_name)
		return ""

	# 清理旧的自动存档
	var cleanup_success = await _clean_old_auto_saves()
	if not cleanup_success:
		push_warning("Failed to clean old auto saves")

	# 发送信号
	auto_save_created.emit(auto_save_name)
...
func _get_auto_save_name() -> String:
	if _current_id:
		return auto_save_prefix + "save_" + _current_id
	if _current_save_name:
		return auto_save_prefix + "save_" + _get_auto_save_id(_current_save_name)
	return auto_save_prefix + _get_timestamp()


func _get_auto_save_id(save_name: String = "") -> String:
	if save_name:
		var names: Array = save_name.split("_")
		if names.is_empty():
...
# 从文件名获取存档ID
func _get_save_name_from_file(file_name: String) -> String:
	return _save_strategy.get_save_name_from_file(file_name)


# 确保存档目录存在
func _ensure_save_directory_exists() -> void:
	var save_dir = Setting.get_setting_value("save_system/defaults/save_directory")
...
func _clean_old_auto_saves() -> void:
	...
	var auto_saves = saves.filter(
		func(save):
			var save_name = save.get("save_name")
			return save_name.begins_with(auto_save_prefix)
	)
...
func _collect_node_states() -> Array[Dictionary]:
	...
	for saveable in saveables:
		if saveable.has_method("save"):
			...
			push_warning("缺少save方法！%s" % str(saveable))
	return nodes
...
func _apply_node_states(nodes: Array) -> void:
	for node_data: Dictionary in nodes:
		...
		if node_path.is_empty():
			push_error("节点路径为空！%s" % str(node_data))
			continue
		...
		if node:
			if node.has_method("load_data"):
				...
				push_warning("缺少 load_data 方法！%s" % str(node))
		else:
			# 节点不存在，缓存数据
			print("节点 %s 尚未加载，缓存其状态数据。" % node_path)
