...
func submit_task(thread_name: StringName, task_func: Callable) -> String:
	...
	if not thread:
		push_error("ModuleThread: Failed to create or get thread '%s'." % thread_name)
		return ""
...
func unload_thread(thread_name: StringName) -> void:
	...
	if _threads.has(thread_name):
		...
		print("ModuleThread: Unloaded thread '%s'." % thread_name)
	else:
		push_warning("ModuleThread: Thread '%s' not found for unloading." % thread_name)
	_mutex.unlock()
...
func clear_threads() -> void:
	...
	print("ModuleThread: Clearing all threads (%d)..." % thread_names.size())
	...
	_threads.clear()
	_mutex.unlock()
	print("ModuleThread: All threads cleared.")
...
func _ensure_thread_exists_internal(thread_name: StringName) -> SingleThread:
	if _threads.has(thread_name):
		...
		print("ModuleThread: Creating new thread '%s'." % thread_name)
