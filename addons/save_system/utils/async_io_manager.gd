...
const ASEEncryptionStrategy = preload("./io_strategies/encryption/aes_encryption_strategy.gd")
...
func _connect_signals() -> void:
	...
	if not _io_thread or not is_instance_valid(_io_thread):
		push_error("AsyncIO: Cannot connect signals, IO thread is not initialized.")
		return
...
func _execute_read_operation(path: String, key_bytes: PackedByteArray) -> Variant:
	...
	if not file:
		push_error("AsyncIO: Failed to open file for reading: %s, Error: %s" % [path, FileAccess.get_open_error()])
		return null
...
func _execute_write_operation(path: String, data: Variant, key_bytes: PackedByteArray) -> bool:
	...
	if not dir.dir_exists(dir_path):
		...
		if err != OK:
			push_error("AsyncIO: Failed to create directory: %s, Error code: %d" % [dir_path, err])
			return false

	# Open and write file
	var file = FileAccess.open(path, FileAccess.WRITE)
	if not file:
		push_error("AsyncIO: Failed to open file for writing: %s, Error: %s" % [path, FileAccess.get_open_error()])
		return false

	var ok := file.store_buffer(processed_bytes)
	if not ok:
		push_error("AsyncIO: Failed to write file: %s" % path)
	file.close()
	return true
...
func _execute_delete_operation(path: String) -> bool:
	if not FileAccess.file_exists(path):
		push_warning("AsyncIO: File to delete does not exist: %s" % path)
		return true

	var dir = DirAccess.open(path.get_base_dir())
	if not dir:
		push_error("AsyncIO: Could not access directory for deletion: %s" % path.get_base_dir())
		return false

	var err := dir.remove(path.get_file())
	if err != OK:
		push_error("AsyncIO: Failed to delete file: %s, Error code: %d" % [path, err])
		return false
...
func _get_file_list(directory_path: String) -> Array:
	...
	if not dir:
		push_error("AsyncIO: Cannot access directory: %s" % directory_path)
		return []
...
func _process_data_for_write(data: Variant, key_bytes: PackedByteArray) -> PackedByteArray:
	...
	if not _serializer:
		push_error("AsyncIO: No serialization strategy set.")
...
func _on_task_completed(result_dict: Dictionary, internal_task_id: int) -> void:  # 注意：这里接收 int ID
	...
	if public_task_id.is_empty():
		push_error("AsyncIO: Received completed signal for unknown internal task ID: %d" % internal_task_id)
		return  # 无法处理，直接返回
	...
	if not result_dict is Dictionary or not result_dict.has("success"):
		push_error("AsyncIO: Internal task result format error for task %s (Internal ID: %d): %s" % [public_task_id, internal_task_id, str(result_dict)])
	...
	print("AsyncIO: Task %s completed (Internal ID: %d). Success: %s" % [public_task_id, internal_task_id, str(success)])
	# 发出公共信号，使用 String ID
